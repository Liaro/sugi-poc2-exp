...
WITH BASE AS(
  SELECT
    *
  FROM
    `{{project_id}}.{{dataset_id}}.diff_monthly_prescription`
  WHERE
    -- 統計量算出に使用するデータまで読み込む
  ...
  SELECT
    ...
    {% for op in ops %}
      {% for day in preceding_days %}
        -- yj_code, store単位の特徴
        {{op}}(diff_total_dose_by_yj_store) OVER (yj_store_window_{{day}}) AS total_dose_by_yj_store_{{op}}_{{day}},
        ...
        -- yj_code単位の特徴
        {{op}}(diff_total_dose_by_yj) OVER (yj_window_{{day}}) AS total_dose_by_yj_{{op}}_{{day}},
  ...
  SELECT
    ...
    {% for day in lag_days %}
      -- yj_code, store単位の特徴
      LAST_VALUE(diff_total_dose_by_yj_store) OVER (yj_store_window_{{day}}) AS lag{{day}}_total_dose_by_yj_store,
      ...
      -- yj_code単位の特徴
      LAST_VALUE(diff_total_dose_by_yj) OVER (yj_store_window_{{day}}) AS lag{{day}}_total_dose_by_yj,
  ...
  LEFT JOIN (
    SELECT
      ...
      LAST_VALUE(diff_total_dose_by_yj_store) OVER (
          PARTITION BY yj_code, store_code  
          ORDER BY UNIX_DATE(dispensing_date)
          {% if is_prediction %}
          RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
      ...
      LAST_VALUE(diff_total_dose_by_yj) OVER (
          PARTITION BY yj_code  
          ORDER BY UNIX_DATE(dispensing_date)
          {% if is_prediction %}
          RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
