...
SELECT
  *,
FROM TRAIN_DATA 
LEFT JOIN `{{project_id}}.train_internal.monthly_target_feature` USING(yj_code, store_code, dispensing_date)
LEFT JOIN `{{project_id}}.train_internal.monthly_category_feature` USING(yj_code, store_code, dispensing_date)
LEFT JOIN `{{project_id}}.train_internal.monthly_holiday_feature` USING(yj_code, store_code, dispensing_date)
